...
> **Три парсера — две разные задачи.** `meon-md` по дизайну **не** соответствует
> CommonMark — он парсит подмножество Markdown в плоские типизированные векторы
> спанов (O(1) доступ на вид элементов, извлечение одного типа через `find_*`,
> zero-copy спаны). `pulldown-cmark` и `comrak` — полный CommonMark, они
> производят поток событий / AST. Разрыв в пропускной способности — это разница
> между этими задачами. `Throughput::Bytes` измеряет, как быстро поглощается
> вход, поскольку три решения производят разное.
...
## meon-md контекстное извлечение (`context()` + `find_context_*`)

`context(source)` строит карту непрозрачных регионов — блоки с кодом, кодовые
спаны, автоссылки — за один потоковый проход; `find_context_*` — тот же
standalone-матчер, но кандидаты-разделители внутри этих регионов пропускаются.
Вариант получает каждое правило, которое само не является непрозрачным;
непрозрачные источники сохраняют только свой контекст-свободный `find_*`. Это
закрывает расхождение по ограждениям/непрозрачности из секции выше — счётчики
`full` vs `context-aware` отчитываются рядом (см.
[`ARCHITECTURE_RU.md §12`](https://github.com/vgnapuga/meon/blob/main/ARCHITECTURE_RU.md#12-standalone-итераторы)).
...
- `find_context_*` — скан по уже построенной карте; поэлементный оверхед
  относительно контекст-свободного `find_*` выше.
