...
| `meon-md_compare`    | meon-md против `pulldown-cmark` / `comrak` — cross-parser пропускная способность. |

Отчёты о составе корпусов, полные таблицы результатов и cross-parser числа
живут в своих документах — этот файл это обзор, как-запускать, рамка честности
и тестовое окружение. Cross-parser сравнение —
...
> элементов обычно ниже**, чем в `hot` или `heavy`, — а меньшая плотность это
> меньше работы на элемент, поэтому реальная пропускная способность обычно
> оказывается **на уровне или выше** чисел `hot`/`heavy`. Читайте `hot`/`heavy`
> как консервативную нижнюю границу, с `plain` (без разметки) как потолком, а
> вашу нагрузку — где-то между.

Точные счётчики элементов по корпусам (отчёт о составе, печатаемый перед каждым
прогоном) — в
[***MD_COMPARE.md***](https://github.com/vgnapuga/meon/blob/main/benches/MD_COMPARE_RU.md).
...
- **Сначала intra-engine.** `meon-md_parse` / `meon-md_standalone` измеряют
  *этот* движок на *этих* корпусах. Они имеют смысл для «регрессировало ли моё
  изменение?» и «насколько помогает AVX2?», но не для таблицы лидеров.
- **Не рейтинг CommonMark.** `meon-md` возвращает плоские спаны для
  *подмножества* Markdown и не строит AST, не резолвит ссылки и не рендерит.
  Cross-parser сравнение против `pulldown-cmark` / `comrak` живёт в
  [***MD_COMPARE.md***](https://github.com/vgnapuga/meon/blob/main/benches/MD_COMPARE_RU.md),
  поданное как разница архитектур, а не рейтинг качества.
...
- **У `max_nest` плоская стоимость на строку.** Стек активных блоков на
  блочном уровне и ограниченные стеки инлайн-движка — это массивы
  `[T; max_nest]`, обнуляемые при **каждом** вызове `parse_block!` /
  `parse_inline!`, независимо от того, вкладывается ли реально эта строка.
  Ставьте `max_nest` в минимальное значение, которое нужно вашей грамматике —
  у `meon-md` это осознанные `4`; больший предел стоит пропускной способности
  на каждой строке с инлайном, вкладывается там что-то или нет.
