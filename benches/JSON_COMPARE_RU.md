...
> **Эти числа демонстрируют архитектурную разницу, а не ранжирование по
> качеству.** `meon-json` по своей сути — **структурный читатель**: он разбирает
> JSON в плоскую таблицу спанов и ничего не валидирует, не парсит числа и не
> разэкранирует строки. `simd-json` и `sonic-rs` — валидирующие парсеры, которые
> материализуют tape / владеющее значение: они парсят каждое число и
> разэкранируют каждую строку. Поэтому разрыв в пропускной способности отражает
> две разные задачи. `Throughput::Bytes` измеряет, как быстро потребляется вход,
> поскольку все четыре производят разные вещи.
...
- **Разный объём работы.** `meon-structural` выдаёт спаны и ничего не
  ...
  разэкранируют каждую строку. meon делает меньше — поэтому читает быстрее.
  Эквивалентная линия тоже парсила бы числа в значения и разэкранировала бы
  строки — **ни одна линия meon этого не делает**, так что даже `meon-typed` не
  эквивалентен по работе tape или владеющему значению.
...
|-----------|---------------------------------------------------------------------------------------------|---------------------------------------------------------------------------|
| `numbers` | Плоский массив чисел / булевых / null.                                                       | Сканирование скаляров. meon делает меньше всех; валидаторы парсят каждое число. |
...
- **Ранжирование переворачивается с масштабом.** На small валидирующие парсеры
...
- Большее число у meon — это «сделал меньше работы» (без валидации, без парсинга
  чисел, без разэкранирования), а не «лучший парсер». Потребитель, которому нужны
